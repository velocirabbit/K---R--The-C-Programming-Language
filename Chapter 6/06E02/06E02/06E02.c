...
typedef enum e_vartype {
    VOID = 0, CHAR, SHORT, INT, LONG, FLOAT, DOUBLE,
...
int findVarNames(varname **vn, char ***lines, int n) {
    ...
    for (int i = 0; i < n; i++) // iterate through each line
        if ((inLine = saveVars((*lines)[i], vn)) > 0)
            total += inLine;
    return total;
}

/**
 * saveVars: Counts the number of variables defined in a line of text, then
 *   puts each into a varname and saves it to the varname array vars.
 *
...
/**
 * isvartype: Checks the given word to see if it's a vartype.
 *
 * Returns: The vartype (int corresponding to index) if it is, -1 if it isn't.
 */
int isvartype(char *w) {
...
/**
 * getword: Gets the next word (alphanumeric and '_' only) in the string. Skips
 *   everything else. Saves the word into string w.
 *
