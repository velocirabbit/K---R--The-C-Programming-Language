...
main() {
    ...
    if (keys == NULL) {
        printf("ERROR: Couldn't initialize variable key names array\n");
        return -1;
    }
    keys[0] = malloc(sizeof(char[MAXOPLEN]));
    if (keys[0] == NULL) {
        printf("ERROR: Couldn't initialize pointer for variable 'ans'\n");
        return -1;
    }
    strcpy(keys[0], ans);
    values = malloc(varCount * sizeof(char*));
    if (values == NULL) {
        printf("ERROR: Couldn't initialize variable values array\n");
        return -1;
    }
    values[0] = malloc(sizeof(char[MAXOPLEN]));
    if (values[0] == NULL) {
        printf("ERROR: Couldn't initialize pointer for value of variable 'ans'\n");
        return -1;
    }
...
int getop(char s[]) {
    ...
    while ((s[i] = (buffPos > 0) ? buff[--buffPos] : getch()) == ' ' || s[i] == '\t');

    if (!isdigit(s[i]) && s[i] != '.') {
        // Is either an operator, a sign, or nothing useful
        if (s[i] == '-' || s[i] == '+') {
            s[++i] = (buffPos > 0) ? buff[--buffPos] : getch();
            if (isspace(s[i])) {
                if (buffPos > MAXBUFFSIZE)
                    printf("ERROR: Character input buffer full\n");
                else
                    buff[buffPos++] = (s[i] == EOF) ? 0 : s[i];
                // The '-' or '+' was just an operator after all
                return s[--i];
            }
        }
        else if (isalpha(s[i])) {
            while (isalpha(s[++i] = tolower((buffPos > 0) ? buff[--buffPos] : getch())));
            if (s[i] != EOF) {
                if (buffPos > MAXBUFFSIZE)
                    printf("ERROR: Character input buffer full\n");
                else
            ...
            switch (m) {
                ...
                case 12: case 13:
                {
                    char c1 = (buffPos > 0) ? buff[--buffPos] : getch();
                    char c2 = (buffPos > 0) ? buff[--buffPos] : getch();
                    if (c1 == '-' && c2 == '>')
                        ...
                        if (c1 != -1 && c2 != -1) {
                            if (buffPos > MAXBUFFSIZE)
                                ...
                                buff[buffPos++] = (c2 == EOF) ? 0 : c2;
                            if (buffPos > MAXBUFFSIZE)
                                printf("ERROR: Character input buffer full\n");
                            else
                                buff[buffPos++] = (c1 == EOF) ? 0 : c1;
                        }
                        printf("ERROR: Syntax for storing or recalling a variable is:\n");
                        printf("         (mem/re)->{name}\n");
                    }
    ...
    if (isdigit(s[i]))
        while (isdigit(s[++i] = (buffPos > 0) ? buff[--buffPos] : getch()));
    if (s[i] == '.')
        while (isdigit(s[++i] = (buffPos > 0) ? buff[--buffPos] : getch()));
...
void memorize(char s[], double n, bool flag) {
    ...
    if (!found) {
        ...
        if (keys == NULL) {
            printf("ERROR: Couldn't reallocate memory for variable keys array\n");
            return;
        }
        ...
        if (values == NULL) {
            printf("ERROR: Couldn't reallocate memory for variable values array\n");
            return;
        }
        values[i] = malloc(sizeof(char[MAXOPLEN]));
        if (values[i] == NULL) {
            printf("ERROR: Couldn't reallocate memory for value of variable key '%s' with value %f\n", s, n);
        }
        snprintf(values[i], MAXOPLEN, "%f", n);
    }
}
